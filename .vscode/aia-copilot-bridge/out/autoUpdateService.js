...
class AutoUpdateService {
    constructor(symbolProvider, contextEnhancer, workspaceRoot) {
    ...
    async performUpdate() {
        ...
        try {
            ...
            if (config.get('showUpdateNotifications', false)) {
                vscode.window.showInformationMessage(`🔄 AIA: Copilot context updated using existing CLI (${duration}ms)`, { modal: false });
            }
        }
        catch (error) {
            console.error('❌ Auto-update failed:', error);
            vscode.window.showWarningMessage(`AIA auto-update failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
        finally {
            this.isUpdating = false;
     ...
     */
    async rebuildSymbolIndexUsingAIA() {
        try {
            // Check if AIA CLI is available
            const aiaPath = path.join(this.workspaceRoot, 'node_modules', '.bin', 'aia');
            const hasLocalAia = await fs.pathExists(aiaPath);
            let command = 'aia index build --force';
            if (hasLocalAia) {
                command = `${aiaPath} index build --force`;
            }
            else if (await fs.pathExists(path.join(this.workspaceRoot, 'dist', 'index.js'))) {
                command = `node ${path.join(this.workspaceRoot, 'dist', 'index.js')} index build --force`;
            }
     ...
     */
    async updateCopilotInstructionsUsingAIA() {
        try {
            // Check if AIA CLI is available
            const aiaPath = path.join(this.workspaceRoot, 'node_modules', '.bin', 'aia');
            const hasLocalAia = await fs.pathExists(aiaPath);
            let command = 'aia index export --type copilot-instructions --output .github/copilot-instructions.md';
            if (hasLocalAia) {
                command = `${aiaPath} index export --type copilot-instructions --output .github/copilot-instructions.md`;
            }
            else if (await fs.pathExists(path.join(this.workspaceRoot, 'main.js'))) {
                command = `node ${path.join(this.workspaceRoot, 'main.js')} index export --type copilot-instructions --output .github/copilot-instructions.md`;
            }
            console.log(`📝 Running: ${command}`);
...
*This file automatically updates when code changes to keep GitHub Copilot context fresh and accurate.*`;
            ...
            console.error('Failed to load symbol index for instructions generation:', error);
...
//# sourceMappingURL=autoUpdateService.js.map