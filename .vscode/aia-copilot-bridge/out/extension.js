...
function activate(context) {
    ...
    context.subscriptions.push(vscode.languages.registerCompletionItemProvider({ scheme: 'file', language: '*' }, symbolProvider, '.'), vscode.commands.registerCommand('aia.rebuildSymbolIndex', async () => {
        ...
        await symbolProvider.buildSymbolIndex();
        vscode.window.showInformationMessage('⚡ Symbol index updated (fast local operation)!');
    }), vscode.commands.registerCommand('aia.updateAIContextOnly', async () => {
        const config = vscode.workspace.getConfiguration('aia.symbolIndex');
        const promptBefore = config.get('promptBeforeExpensiveUpdates', true);
        if (promptBefore) {
            const choice = await vscode.window.showWarningMessage('⚠️ AI Context Update\n\nThis operation uses API credits and may take 30-60 seconds.\n\nContinue?', { modal: true }, 'Yes, Update', 'Cancel');
            if (choice !== 'Yes, Update') {
                return;
            }
        }
        await contextEnhancer.enhanceCopilotContext();
        vscode.window.showInformationMessage('🧠 AI context updated (expensive operation completed)!');
    }), vscode.commands.registerCommand('aia.toggleAIUpdates', async () => {
        const config = vscode.workspace.getConfiguration('aia.symbolIndex');
        const currentValue = config.get('enableAIUpdates', false);
        if (!currentValue) {
            // Warn when enabling AI updates
            const choice = await vscode.window.showWarningMessage('⚠️ Enable AI Auto-Updates?\n\nThis will automatically trigger expensive AI operations that use API credits.\n\nRecommended: Use "smart" strategy to minimize costs.', { modal: true }, 'Enable (Smart)', 'Enable (Manual Only)', 'Cancel');
            if (choice === 'Cancel' || !choice) {
                return;
            }
            await config.update('enableAIUpdates', true, vscode.ConfigurationTarget.Workspace);
            if (choice === 'Enable (Smart)') {
                await config.update('aiUpdateStrategy', 'smart', vscode.ConfigurationTarget.Workspace);
            }
            vscode.window.showInformationMessage(`✅ AI auto-updates enabled with ${choice === 'Enable (Smart)' ? 'smart' : 'manual'} strategy`);
        }
        else {
            await config.update('enableAIUpdates', false, vscode.ConfigurationTarget.Workspace);
        ...
        const enableAIUpdates = config.get('enableAIUpdates', false);
        const aiStrategy = config.get('aiUpdateStrategy', 'manual');
        const symbolDelay = config.get('symbolDebounceDelay', 2000);
        const aiDelay = config.get('aiDebounceDelay', 60000);
        const maxDaily = config.get('maxDailyAIUpdates', 10);
        const smartThreshold = config.get('smartUpdateThreshold', 5);
        const promptBefore = config.get('promptBeforeExpensiveUpdates', true);
...
🔧 To modify: Go to Settings → Extensions → AIA Symbol Index`;
        ...
        const choice = await vscode.window.showWarningMessage('Reset daily AI update counter?\n\nThis will allow more automatic AI updates today.', { modal: true }, 'Reset', 'Cancel');
        if (choice === 'Reset') {
        ...
        const choice = await vscode.window.showQuickPick([
            ...
            {
                label: '🧠 Smart Updates',
                description: 'AI updates when significant architecture changes detected',
        ...
        if (!choice)
            return;
        let strategy = 'manual';
        if (choice.label.includes('Smart'))
            strategy = 'smart';
        if (choice.label.includes('Time-Based'))
            strategy = 'time-based';
        await config.update('aiUpdateStrategy', strategy, vscode.ConfigurationTarget.Workspace);
        if (strategy !== 'manual') {
            await config.update('enableAIUpdates', true, vscode.ConfigurationTarget.Workspace);
        }
        vscode.window.showInformationMessage(`Update strategy set to: ${strategy}`);
    }));
