...
exports.SymbolIndexPerformanceMonitor = void 0;
const vscode = require("vscode");
class SymbolIndexPerformanceMonitor {
    constructor() {
        this.metrics = new Map();
        this.outputChannel = vscode.window.createOutputChannel('AIA Symbol Index Performance');
    }
    trackLookup(symbolName, duration) {
    ...
    updateStatusBar() {
        ...
        vscode.window.setStatusBarMessage(`⚡ AIA Symbol Lookup: ${avg.toFixed(1)}ms avg (${improvement}x faster)`, 5000);
    }
    reportPerformance() {
        const lookups = this.metrics.get('lookups') || [];
        ...
        const avgBuildTime = builds.length > 0 ? builds.reduce((a, b) => a + b, 0) / builds.length : 0;
...
`;
    ...
    getRecommendedRebuildInterval() {
        const builds = this.metrics.get('builds') || [];
        if (builds.length < 2)
            return 'On file save';
        const avgBuildTime = builds.reduce((a, b) => a + b, 0) / builds.length;
        if (avgBuildTime < 1000)
            return 'On file save';
        if (avgBuildTime < 3000)
    ...
    getMetrics() {
        ...
        return {
            ...
            averageLookupTime: lookups.length > 0
            ...
            averageBuildTime: builds.length > 0
                ...
                : 0,
            performanceGain: lookups.length > 0
                ? 50 / (lookups.reduce((a, b) => a + b, 0) / lookups.length)
                : 1,
        };
    }
}
exports.SymbolIndexPerformanceMonitor = SymbolIndexPerformanceMonitor;
