...
exports.AIASymbolIndexProvider = void 0;
...
class AIASymbolIndexProvider {
    constructor() {
        this.symbolIndex = new Map();
        this.lookupStats = { totalLookups: 0, totalTime: 0 };
    }
    ...
    async buildSymbolIndex() {
        ...
        try {
            ...
            else if (await fs.pathExists(path.join(workspaceRoot, 'dist', 'index.js'))) {
                // Use local build if available
                command = `node ${path.join(workspaceRoot, 'dist', 'index.js')} index symbols:build --force`;
            }
    ...
    findRelevantSymbols(context) {
        ...
        for (const [name, data] of this.symbolIndex) {
            ...
            if (results.length >= 15)
                break; // Limit results to avoid overwhelming
        }
    ...
    createCompletionItem(symbol) {
        const item = new vscode.CompletionItem(symbol.name, this.getCompletionItemKind(symbol.info?.type || symbol.type));
        ...
        if (symbol.needsImport && symbol.importPath) {
            item.additionalTextEdits = [
                vscode.TextEdit.insert(new vscode.Position(0, 0), `import { ${symbol.name} } from '${symbol.importPath}';\\n`),
            ];
        }
        // Set sort text to prioritize based on relevance
        item.sortText = `${symbol.info?.metadata?.usageCount || 0}`.padStart(5, '0');
        return item;
    }
    ...
    generateSymbolDocumentation(symbol) {
        ...
        if (location) {
            md.appendMarkdown(`📍 Location: [${location.file}:${location.line}](${location.file})\\n\\n`);
        }
        if (symbolInfo.metadata?.description) {
            md.appendMarkdown(`📝 Description: ${symbolInfo.metadata.description}\\n\\n`);
        }
        if (symbolInfo.relationships) {
            md.appendMarkdown('**Relationships:**\\n');
            if (symbolInfo.relationships.extends?.length > 0) {
                md.appendMarkdown(`- Extends: ${symbolInfo.relationships.extends.join(', ')}\\n`);
            }
            if (symbolInfo.relationships.implements?.length > 0) {
                md.appendMarkdown(`- Implements: ${symbolInfo.relationships.implements.join(', ')}\\n`);
            }
            if (symbolInfo.relationships.usedBy?.length > 0) {
                md.appendMarkdown(`- Used by: ${symbolInfo.relationships.usedBy.length} symbols\\n`);
            }
        }
    ...
    async getSymbolStats() {
        const avgLookupTime = this.lookupStats.totalLookups > 0
            ? (this.lookupStats.totalTime / this.lookupStats.totalLookups).toFixed(2)
            : '0';
        return {
...
exports.AIASymbolIndexProvider = AIASymbolIndexProvider;
