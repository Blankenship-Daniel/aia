...
export class CopilotContextEnhancer {
  ...
  private extractSymbols(): Map<string, any> {
  ...
  async enhanceCopilotContext(): Promise<void> {
    ...
    if (await fs.pathExists(indexPath)) {
      ...
      console.warn(
        'Symbol index not found, skipping Copilot context enhancement'
      );
    }
  }
...
`;
  ...
  private generateSymbolOverview(): string {
  ...
  private generateRelationshipMap(): string {
    ...
    for (const [name, data] of Object.entries(this.symbolIndex.symbols || {})) {
      ...
      if (symbolInfo.relationships?.extends?.length > 0) {
        relationships.push(
          `${name} extends ${symbolInfo.relationships.extends.join(', ')}`
        );
      }
      if (symbolInfo.relationships?.implements?.length > 0) {
        relationships.push(
          `${name} implements ${symbolInfo.relationships.implements.join(', ')}`
        );
      }
      if (
        symbolInfo.relationships?.usedBy?.length > 0 &&
        symbolInfo.relationships.usedBy.length > 5
      ) {
        relationships.push(
          `${name} is heavily used (${symbolInfo.relationships.usedBy.length} references)`
        );
      }
    }
    ...
    if (this.symbolIndex?.symbols) {
      for (const [name, data] of Object.entries(this.symbolIndex.symbols)) {
        ...
        if (symbolInfo.relationships?.extends) {
          symbolInfo.relationships.extends.forEach((parent: string) => {
            graph.edges.push({
              from: name,
              to: parent,
              type: 'extends',
        ...
        if (symbolInfo.relationships?.implements) {
          symbolInfo.relationships.implements.forEach((iface: string) => {
            graph.edges.push({
              from: name,
              to: iface,
              type: 'implements',
        ...
        if (symbolInfo.relationships?.uses) {
          symbolInfo.relationships.uses.forEach((used: string) => {
            graph.edges.push({
              from: name,
              to: used,
              type: 'uses',
